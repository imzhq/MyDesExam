...
public class DES {
	private static int[] INITIAL_PERM = { 58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36, 28, 20, 12, 4, 62, 54, 46, 38,
			30, 22, 14, 6, 64, 56, 48, 40, 32, 24, 16, 8, 57, 49, 41, 33, 25, 17, 9, 1, 59, 51, 43, 35, 27, 19, 11, 3,
			61, 53, 45, 37, 29, 21, 13, 5, 63, 55, 47, 39, 31, 23, 15, 7 };
	private static int[] FINAL_PERM = { 40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15, 55, 23, 63, 31, 38, 6, 46, 14, 54,
	...
	private static int[] PERM = { 16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23, 26, 5, 18, 31, 10, 2, 8, 24, 14, 32, 27, 3,
			9, 19, 13, 30, 6, 22, 11, 4, 25 };
	private static String input = "E:\\\\Desktop\\\\txt.txt";
	private static String output = "65A63178904AC0E8";
	 ...
	 * 
	 * @param args
	 *            gives the flags and file to be read and either encrypted or
	 *            decrypted; see checkFlag(string []) for more details.
	 */

	public static void main(String[] args) {
		encrypt("", "33313345333133453334334133303034", "1C4A029B156E257A80572F6B4692CBCB");
	}

	/**
	 * genKey() - private, void
	 * 
	 * Generates a 64 bit key using the SecureRandom() class then using the
	 * BigInteger class, prints a 16 length hexadecimal value that becomes the key
	 ...
	 * 
	 * using the given key, encrypts the message within the file and prints encoded
	 * blocks line by line into the output file
	 */
	private static void encrypt() {
		...
		encrypt(message, inputS, keyH);
	}

	public static void encrypt(String message, String inputS, String keyH) {
		// try {
		...
		for (int i = 0; i < inbin.length; i++) {
			...
			inbin[i] = "";
			for (int j = 0; j < 8; j++) {
				inbin[i] += ("00000000" + Integer.toBinaryString(bit8.charAt(j)))
						.substring(Integer.toBinaryString(bit8.charAt(j)).length());
			}
		}
		...
		for (int i = 0; i < inbin.length; i++) {
			...
			for (int j = 0; j < 16; j++) {
				...
				for (int next = 0; next < 48; next += 6) {
					...
					if (next / 6 == 0) {
						sboxed += ("0000" + Integer.toString(s1[row][col], 2))
								.substring(Integer.toString(s1[row][col], 2).length());
					} else if (next / 6 == 1) {
						sboxed += ("0000" + Integer.toString(s2[row][col], 2))
								.substring(Integer.toString(s2[row][col], 2).length());
					} else if (next / 6 == 2) {
						sboxed += ("0000" + Integer.toString(s3[row][col], 2))
								.substring(Integer.toString(s3[row][col], 2).length());
					} else if (next / 6 == 3) {
						sboxed += ("0000" + Integer.toString(s4[row][col], 2))
								.substring(Integer.toString(s4[row][col], 2).length());
					} else if (next / 6 == 4) {
						sboxed += ("0000" + Integer.toString(s5[row][col], 2))
								.substring(Integer.toString(s5[row][col], 2).length());
					} else if (next / 6 == 5) {
						sboxed += ("0000" + Integer.toString(s6[row][col], 2))
								.substring(Integer.toString(s6[row][col], 2).length());
					} else if (next / 6 == 6) {
						sboxed += ("0000" + Integer.toString(s7[row][col], 2))
								.substring(Integer.toString(s7[row][col], 2).length());
					} else {
						sboxed += ("0000" + Integer.toString(s8[row][col], 2))
								.substring(Integer.toString(s8[row][col], 2).length());
					}
				}
	 ...
	 * 
	 * using the given key, decrypts the message within the file and prints the
	 ...
	 * 
	 * Create the 16 keys that will be used for the DES implementation, for more
	 * information please see DES wiki on Key Schedules.
	 */
	 ...
	 * 
	 * simple method to check for and process correct input given from the command
	 * line.
	 * 
	 * @param args
	 *            - flags given on command line.
	 * 
	 *            will be reworking this at a later time to give the program more
	 *            versatility.
	 */
	private static void checkFlag(String[] args) {
		...
		// :"
		// + "Encrypts the input file and places the encrypted file into the specified
		// output file ");
		// System.out.println("-d <64 bit key in hex> -i <input file> -o <output file>
		// :"
		// + "Decrypts the input file and places the decrypted file into the specified
		// output file ");
		// help = true;
