...
public class Util {
	...
	/*
	 * // private static byte uniteBytes(byte src0, byte src1) { byte _b0 =
	 * Byte.decode("0x" + new String(new byte[] { src0 })) .byteValue(); _b0 =
	 * (byte) (_b0 << 4); byte _b1 = Byte.decode("0x" + new String(new byte[] { src1
	 * })) .byteValue(); byte ret = (byte) (_b0 ^ _b1); return ret; }
	 */
	 ...
	 * 
	 * @param srcdata
	 *            ascii型数组 @return result 转换后的hex型数组
	 * @param len
	 */
	 ...
	 * 
	 * @param srcdata
	 *            hex型数组 @return result 转换后的ascii型数组
	 */
	 ...
	 * 
	 * @param value
	 *            int型数值 @return result 转换后的4字节BYTE[]
	 */
	...
	/**
	 * 将一个byte[]型值转为int型,长度不大于4； @param srcnum[]
	 * 
	 * @param srcnum[]
	 *            offset
	 * @param len
	...
	/**
	 * 将一个byte[]型的DCD值转为String型； @param byte[] value 存储BCD格式的数组 @return 一个新的String值
	 */
