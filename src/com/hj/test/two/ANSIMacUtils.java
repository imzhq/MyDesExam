 ...
 * 
 * 在 JDK 1.4里，已包含一个 Mac 类（javax.crypto.Mac），可以生成MAC。 但它是参照HMAC（Hash-based
 * Message Authentication Code，基于散列的消息验证代码）实现的。 有时，需要采用ANSI-X9.9算法计算MAC。
 * 
 * 1. 算法描述 参与ANSI X9.9 MAC计算的数据主要由三部分产生：初始数据、原始数据、补位数据。 1) 算法定义：采用DEC
 * CBC（zeroICV）或ECB算法 2) 初始数据：0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00 3) 原始数据:
 * 4) * 补位数据：若原始数据不是8的倍数,则右补齐0x00；若原始数据位8的整数倍，则不用补齐0x00。 5) 密钥: MAC密钥
 * 
 * MAC的产生由以下方式完成：(最后一组数据长度若不足8的倍数，则右补齐0x00；若数据长度为8的整数倍，则无需补充0x00) 初始数据 BLOCK #1
 * BLOCK #2 BLOCK #3 ... BLOCK #N | | | | | +-----> XOR +---> XOR +---> XOR
 * +---> XOR | | | | | | | DES ---+ DES ---+ DES ---+ DES ---> MAC | | | | KEY
 * KEY KEY KEY
 * 
 * 返回 -- 加密后的缓冲区
 */
public class ANSIMacUtils {
	...
	public static byte[] calculateANSIX9_19MAC(byte[] key, byte[] data) throws GeneralSecurityException {
		...
		if (key.length != 16) {
			...
			byte[] souce = new byte[16];
			System.arraycopy(key, 0, souce, 0, 16);
			key = souce;
		}
