 ...
 * <p/>
 * The following resources proved valuable in developing and testing this code:
 * <p/>
...
public class newDES {
	...
	/**
	 * Input Permutation. The message block is permuted by this permutation at the
	 * beginning of the algorithm.
	 */
	private static final byte[] IP = { 58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36, 28, 20, 12, 4, 62, 54, 46, 38, 30,
			22, 14, 6, 64, 56, 48, 40, 32, 24, 16, 8, 57, 49, 41, 33, 25, 17, 9, 1, 59, 51, 43, 35, 27, 19, 11, 3, 61,
			53, 45, 37, 29, 21, 13, 5, 63, 55, 47, 39, 31, 23, 15, 7 };

	/**
	 * Final Permutation. The final result is permuted by this permutation to
	 * generate the final ciphertext block.
	 */
	private static final byte[] FP = { 40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15, 55, 23, 63, 31, 38, 6, 46, 14, 54,
	...
	/**
	 * Expansion Permutation. The Feistel function begins by applying this
	 * permutation to its 32-bit input half-block to create an "expanded" 48-bit
	 * value.
	 */
	private static final byte[] E = { 32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9, 8, 9, 10, 11, 12, 13, 12, 13, 14, 15, 16, 17,
			16, 17, 18, 19, 20, 21, 20, 21, 22, 23, 24, 25, 24, 25, 26, 27, 28, 29, 28, 29, 30, 31, 32, 1 };

	/**
	 * Substitution Boxes. A crucial step in the Feistel function is to perform bit
	 * substitutions according to this table. A 48-bit value is split into 6-bit
	 * sections, and each section is permuted into a different 6-bit value according
	 * to these eight tables. (One table for each section.)
	 * <p/>
	 * According to Wikipedia: "The S-boxes provide the core of the security of DES
	...
	/**
	 * "P" Permutation. The Feistel function concludes by applying this 32-bit
	 * permutation to the result of the S-box substitution, in order to spread the
	 * output bits across 6 different S-boxes in the next round.
	 */
	private static final byte[] P = { 16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23, 26, 5, 18, 31, 10, 2, 8, 24, 14, 32, 27,
			3, 9, 19, 13, 30, 6, 22, 11, 4, 25 };

	// Permutations relating to subkey generation

	/**
	 * PC1 Permutation. The supplied 64-bit key is permuted according to this table
	 * into a 56-bit key. (This is why DES is only a 56-bit algorithm, even though
	 * you provide 64 bits of key material.)
	 */
	private static final byte[] PC1 = { 57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18, 10, 2, 59, 51, 43, 35, 27,
			19, 11, 3, 60, 52, 44, 36, 63, 55, 47, 39, 31, 23, 15, 7, 62, 54, 46, 38, 30, 22, 14, 6, 61, 53, 45, 37, 29,
			21, 13, 5, 28, 20, 12, 4 };

	/**
	 * PC2 Permutation. The subkey generation process applies this permutation to
	 * transform its running 56-bit keystuff value into the final set of 16 48-bit
	 * subkeys.
	 */
	private static final byte[] PC2 = { 14, 17, 11, 24, 1, 5, 3, 28, 15, 6, 21, 10, 23, 19, 12, 4, 26, 8, 16, 7, 27, 20,
			13, 2, 41, 52, 31, 37, 47, 55, 30, 40, 51, 45, 33, 48, 44, 49, 39, 56, 34, 53, 46, 42, 50, 36, 29, 32 };

	/**
	 * Subkey Rotations. Part of the subkey generation process involves rotating
	 * certain bit-sections of the keystuff by either one or two bits to the left.
	 * This table specifies how many bits to rotate left for each of the 16 steps.
	 */
	...
	/**
	 * Permute an input value "src" of srcWidth bits according to the supplied
	 * permutation table. (Note that our permutation tables, supplied by Wikipedia,
	 * start counting with the left-most bit as "1".)
	 */
	...
	/**
	 * Permute the supplied 6-bit value based on the S-Box at the specified box
	 * number. (Box numbers start at 1, to be consistent with the literature.)
	 */
	...
	/**
	 * Utility method to convert 8 bytes (starting at the specified offset to the
	 * supplied byte array) into a single 64-bit long value. If the supplied byte
	 * array does not contain 8 elements starting at offset, the missing bytes are
	 * regarded as zero padding.
	 */
	...
	/**
	 * Utility method to convert a 64-bit long value into eight bytes, which are
	 * written into the supplied byte array at the specified offset. If the
	 * destination byte array does not have eight bytes starting at offset, the
	...
	/**
	 * Generate 16 48-bit subkeys based on the provided 64-bit key value.
	 */
	...
	/**
	 * Encrypt a 64-bit block of plaintext message into a 64-bit ciphertext.
	 */
	...
	/**
	 * Wrapper around encryptBlock() that allows arguments to be byte arrays instead
	 * of longs.
	 */
	...
	/**
	 * Encrypt the supplied message with the provided key, and return the
	 * ciphertext. If the message is not a multiple of 64 bits (8 bytes), then it is
	 * padded with zeros.
	 * <p/>
	 * This method uses the Electronic Code Book (ECB) mode of operation -- each
	 * 64-bit block is encrypted individually with the same key.
	 */
	 ...
	 * 64-bit block is encrypted individually with the same key.
	 * <p/>
	 * The provided password is converted into a key with the bits of each byte
	 * reversed, to generate a stronger key. See passwordToKey() for more details.
	 */
	...
	/**
	 * Convert a password string into a byte array, reversing the bits of each byte
	 * to place more useful key bits into non-discarded bit-positions of the 64-bit
	 * DES key input. The ever-popular 7-bit ASCII characters have useful
	 * information in the least significant bit which is discarded by DES, and
	 * always have zero in the most significant bit, so reversing the bit order of
	 * the password bytes results in a stronger key.
	 * <p/>
	 * This is consistent with the "VNC Authentication" scheme used in the RFB
	 * protocol:
	 * <p/>
	 * "The RFB specification says that VNC authentication is done by receiving a 16
	 * byte challenge, encrypting it with DES using the user specified password, and
	 * sending back the resulting 16 bytes. The actual software encrypts the
	 * challenge with all the bit fields in each byte of the password mirrored." -
	...
	public static void main(String[] args) {
		...
		test(parseBytes("a4b2 c9ef 0876 c1ce 438d e282 3820 dbde"),
				parseBytes("fa60 69b9 85fa 1cf7 0bea a041 9137 a6d3"), "mypass");
		test(parseBytes("f3ed a6dc f8b7 9dd6 5be0 db8b 1e7b a551"),
				parseBytes("b669 d033 6c3f 42b7 68e8 e937 b4a5 7546"), "mypass");
		...
		test(parseBytes("3331334533313345"), parseBytes("EA86E04B5BD4C5F0E6C372BC72FF2B6C"),
				parseBytes("1C4A029B156E257A80572F6B4692CBCB"));
	 ...
	 * padded with zeros.
	 * <p/>
	 * This method uses the Cypher Block Chaining mode (CBC) mode of operation --
	 * each 64-bit block is XORed with the previous ciphertext before being
	...
	/**
	 * Decrypt a 64-bit block of ciphertext into a 64-bit plaintext message.
	 */
	...
	/**
	 * Wrapper around decryptBlock() that allows arguments to be byte arrays instead
	 * of longs.
	 */
	public static void decryptBlock(byte[] ciphertext, int ciphertextOffset, byte[] message, int messageOffset,
			byte[] key) {
	...
	/**
	 * Decrypt the supplied ciphertext with the provided key, and return the
	 * message. If the ciphertext is not a multiple of 64 bits (8 bytes), then it is
	 * padded with zeros.
	 * <p/>
	 * This method uses the Electronic Code Book (ECB) mode of operation -- each
	 * 64-bit block is encrypted individually with the same key.
	 ...
	 * <p/>
	 * This method uses the Cypher Block Chaining mode (CBC) mode of operation --
	 * each 64-bit block is XORed with the previous ciphertext after being decrypted
	 * with the same key.
	 */
