...
class Oneclass {
	...
	private static final byte[] ST = { 32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9, 8, 9, 10, 11, 12, 13, 12, 13, 14, 15, 16,
	...
	private static final byte[][] S = {
			...
			{ 13, 2, 8, 4, 6, 15, 11, 1, 10, 9, 3, 14, 5, 0, 12, 7, 1, 15, 13, 8, 10, 3, 7, 4, 12, 5, 6, 11, 0, 14, 9,
					...
					5, 6, 11 } }; // end S table array

	// permutation table
	private static final byte[] PT = { 16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23, 26, 5, 18, 31, 10, 2, 8, 24, 14, 32,
	...
	private static final byte[] KP1 = { 57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18, 10, 2, 59, 51, 43, 35, 27,
	...
	private static final byte[] KP2 = { 14, 17, 11, 24, 1, 5, 3, 28, 15, 6, 21, 10, 23, 19, 12, 4, 26, 8, 16, 7, 27, 20,
	...
	private static int[] DES(int[] plaintext, int[] key) {
		...
		/*
		 * System.out.println("{"); for(i = 0; i< ciphertext.length; i++){
		 * System.out.print("" + ciphertext[i] + ","); } System.out.println("}");
		 */
	...
	/*
	 * generate Rn from fiestel function. Parameter are a key kn of 48 bits and a
	...
	private static int[] fiestel(int[] R, int[] roundKey) {
		...
		// final step transform 6_bit 8 blocks xor result to 4_bits 8 blocks using S
		// boxes.
	...
	private static int[] S_Box(int[] input) {
		...
		// 48 bit input will be divided into 8 blocks and each contains 6 bit.
		// Therefore, there
		// will be 8 iteration.
		for (int i = 0; i < 8; i++) {
			// For each 6 bits block input, the 1st and 6th bits will be the row found in S
			// table.
			int row[] = new int[2];
			row[0] = input[6 * i];
			row[1] = input[(6 * i) + 5];
	...
	/*
	 * left_shift function rotates two keys array C[]and D[] by using left_shift
	 * table.
	 */
